...
public sealed class LangKeyGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor InvalidConfiguration = new(
        "LANGKEY004",
        "LangKey generator configuration is invalid",
        "LangKeyGeneratorEnabled must be 'auto', 'true', or 'false'; found '{0}'",
        "LangKey",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true
    );
        ...
        var configuration = context.AnalyzerConfigOptionsProvider.Select(
            static (options, _) =>
                new GeneratorConfiguration(
                    GetGlobalOption(options, "build_property.LangKeyNamespace"),
                    GetGlobalOption(options, "build_property.LangKeyGeneratorEnabled")
                )
        );
    ...
    )
    {
        ...
        )
        {
            context.ReportDiagnostic(
                Diagnostic.Create(
                    InvalidDocument,
                    Location.None,
                    $"uses invalid LangKeyNamespace '{configuration.TargetNamespace}'"
                )
            );
            return;
        }
        ...
        if (!string.IsNullOrWhiteSpace(configuration.TargetNamespace))
        {

...
internal static partial class LangKeyValidation
{
    ...
    public static void ValidateKey(string key, string parameterName)
    {
        if (!IsValidKey(key))
        {
            throw new ArgumentException(
                "A LangKey key must be a non-keyword C# identifier containing only ASCII letters, digits, and underscores; dots are not allowed.",
                parameterName
            );
        }
    }
    ...
    public static string NormalizeCulture(string culture, string parameterName)
    {
        ...
        {
            throw new ArgumentException(
                "A culture must contain non-empty letter or digit subtags separated by '-' or '_'.",
                parameterName
            );
        }
